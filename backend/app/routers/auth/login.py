...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    if utype == "EMPLOYEE":
        ...
        if member.emp_id:
            row = db.execute(
                select(Employee.dept_id, Employee.role_id, Employee.name, Employee.email, Employee.mobile)
        ...
        if member.ext_id:
            row = db.execute(
                select(External.dept_id, External.role_id, External.name, External.email, External.mobile)
    ...
    if dept_id is None or role_id is None:
        raise HTTPException(status_code=500, detail="권한 정보(부서/직책)가 누락되었습니다.")
