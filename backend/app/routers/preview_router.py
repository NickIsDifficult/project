...
from app.models import Event, Notice, Notification, Project
...
def preview_notices(db: Session = Depends(get_db)):
    ...
    return [{"title": n.title, "summary": n.body[:60], "created_at": n.created_at} for n in notices]
...
def preview_notifications(db: Session = Depends(get_db)):
    notis = db.query(Notification).order_by(Notification.created_at.desc()).limit(5).all()
    return [
        {
            "title": n.type.value,
