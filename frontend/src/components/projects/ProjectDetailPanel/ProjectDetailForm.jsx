...
const transformTaskFromApi = t => {
  ...
  task.start_date = task.start_date ?? task.startDate ?? task.start_date ?? task.start_at ?? "";
  task.end_date = task.end_date ?? task.endDate ?? task.due_date ?? task.end_at ?? "";
...
const normalizeTask = (t, projectId) => {
  ...
  if (!Array.isArray(safe.assignee_ids)) {
    safe.assignee_ids = Array.isArray(safe.assignees) ? safe.assignees.map(a => Number(a)) : [];
  } else {
...
const normalizeProjectForSave = (project, fallbackProjectId) => {
  ...
  const tasks = Array.isArray(safe.task) ? safe.task.map(t => normalizeTask2(t, pid)) : [];
            ...
            onChange={async e => {
              ...
              try {
                ...
                const res = await API.post(
                  `/projects/${project.project_id}/attachments`,
                  formData,
                  {
                ...
                // ✅ 업로드 후 최신 목록 새로고침
                const listRes = await API.get(`/projects/${project.project_id}/attachments`);
                setProject(prev => ({
                  ...prev,
                  attachments: listRes.data || [],
                }));
              } catch (err) {
                console.error("❌ 프로젝트 파일 업로드 실패:", err);
                ...
                {isEditing && (
                  <button
                    onClick={async () => {
                      try {
                        await API.delete(
                          `/projects/${project.project_id}/attachments/${f.attachment_id}`,
                        );
                        ...
                        setProject(prev => ({
                          ...
                          attachments: prev.attachments.filter(
                            att => att.attachment_id !== f.attachment_id,
                          ),
                        }));
                      } catch (err) {
        ...
        {rootTasks.length > 0 ? (
          rootTasks.map((task, i) => (
            <TaskNode
                ...
                setProject(prev => {
                  ...
                  const updateRecursive = tasks =>
                    tasks.map(t => {
                      ...
                      if (t.subtask?.length) return { ...t, subtask: updateRecursive(t.subtask) };
                      return t;
                    });
              ...
              onCancel={() => {
                // 새로 추가된(빈 제목) 업무는 목록에서 제거
                if (!task.title.trim()) {
                  setProject(prev => ({
                    ...prev,
                    task: prev.task.filter(t => t.task_id !== task.task_id),
                  }));
                }
              }}
            />
